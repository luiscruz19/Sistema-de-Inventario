...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    const result = await serviceRequest({ method: 'PUT', baseUrl: CONFIG.PRODUCTOS_SERVICE_URL, path: `/categories/${id}`, token, data });
    return NextResponse.json(result);
}
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    const { token } = await getApiHeaders();
    const { id } = await params;
    const result = await serviceRequest({ method: 'DELETE', baseUrl: CONFIG.PRODUCTOS_SERVICE_URL, path: `/categories/${id}`, token });
    return NextResponse.json(result);
}
